...
Debug-сборка ставится как отдельное приложение `com.findev.fintrack.debug` («FinTrack
dev») и живёт рядом с релизной, не конфликтуя с ней.
...
Тег вида `v1.2.3` запускает сборку: тесты, подписанный APK, публикация релиза.
`versionCode` и `versionName` выводятся из тега, поэтому опубликованная сборка не может
разойтись со своим номером.
