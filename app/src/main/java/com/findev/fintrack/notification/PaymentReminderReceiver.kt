...
const val EXTRA_PAYMENT_NAME = "payment_name"
const val EXTRA_AMOUNT_MINOR = "amount_minor"

/**
 * Says a payment is due today. It never records anything: only the user can say what was
...
class PaymentReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // This payment's alarm has just been spent. Rescheduling now would only re-arm the
        // same past date, so the next one is set when the user marks this one paid - which
        // is what changes the answer. Rebuilding here keeps every *other* payment armed
        // after the process was started cold by this broadcast.
        val pending = goAsync()
    ...
    private fun notify(context: Context, id: String, name: String, amountMinor: Long) {
        ...
        val notification = NotificationCompat.Builder(context, REMINDER_CHANNEL_ID)
            ...
            .setContentTitle(context.getString(R.string.reminder_title, name))
            .setContentText(
                context.getString(
