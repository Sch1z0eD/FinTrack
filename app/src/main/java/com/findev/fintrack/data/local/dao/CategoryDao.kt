...
interface CategoryDao {
    ...
    @Query("SELECT * FROM category WHERE is_deleted = 0 ORDER BY is_archived, name")
    fun observeAll(): Flow<List<CategoryEntity>>

    /** Only categories offered in the quick-entry grid. */
    @Query(
        "SELECT * FROM category WHERE is_deleted = 0 AND is_archived = 0 AND type = :type ORDER BY name",
    )
    ...
    @Query("SELECT * FROM category WHERE is_deleted = 0 AND is_archived = 0 AND type = :type ORDER BY name")
