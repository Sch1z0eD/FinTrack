...
class StatPeriodTest {
    ...
    fun thisMonthSpansTheWholeCalendarMonth() {
        val (from, to) = requireNotNull(StatPeriod.THIS_MONTH.range(LocalDate.of(2026, 7, 18)))
    ...
    fun thisMonthEndsOnAShortMonthsLastDay() {
        val (from, to) = requireNotNull(StatPeriod.THIS_MONTH.range(LocalDate.of(2026, 2, 10)))
    ...
    fun lastMonthIsThePrecedingCalendarMonth() {
        val (from, to) = requireNotNull(StatPeriod.LAST_MONTH.range(LocalDate.of(2026, 3, 15)))
        assertEquals(day("2026-02-01"), from)
        assertEquals(day("2026-02-28"), to)
    }
    ...
    fun lastMonthCrossesTheYearBoundary() {
        val (from, to) = requireNotNull(StatPeriod.LAST_MONTH.range(LocalDate.of(2026, 1, 5)))
    ...
    fun thisYearSpansJanuaryToDecember() {
        val (from, to) = requireNotNull(StatPeriod.THIS_YEAR.range(LocalDate.of(2026, 7, 18)))
